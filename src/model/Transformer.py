...
class MultiHead(nn.Module):
    ...
    def _multiheadAttention(self, V, K, Q, head_V, head_K, head_Q, h, scale_factor, W_o):
        ...
        for i in range(h):
            att = self._attention(head_V[i](V), head_K[i](K), head_Q[i](Q), scale_factor)
...
class FeedForward(nn.Module):
    def __init__(self, in_size, filter_size, dropout=0.2):
        super(FeedForward, self).__init__()
        self.feedforward = nn.Sequential(
            nn.Linear(in_size, filter_size),
            GELU(),
            nn.Dropout(dropout),
            nn.Linear(filter_size, in_size)
        )
        initialize_weight(self.feedforward)
...
class EncoderLayer(nn.Module):
    ...
    Args(init):
        in_size: the dimension of the input feature. (Here we don't need to rearrange the matrix.)
    ...
    def __init__(self, in_size, filter_size, dropout=0.2):
        ...
        self.attention_norm = nn.LayerNorm(in_size, eps=1e-7)
        self.attention = MultiHead(in_size)
        self.attention_dropout = nn.Dropout(dropout)
        
        self.ffn_norm = nn.LayerNorm(in_size, eps=1e-7)
        self.ffn = FeedForward(in_size, filter_size, dropout)
...
class Encoder(nn.Module):
    def __init__(self, in_size, filter_size, dropout=0.2, n_layers=1):
        super(Encoder, self).__init__()
        encoders = [EncoderLayer(in_size, filter_size, dropout) for _ in range(n_layers)]
        self.encoders = nn.ModuleList(encoders)
        self.final_norm = nn.LayerNorm(in_size)
...
if __name__ == "__main__":
    ...
    Attention = MultiHead(192, 2)
    v = torch.rand((32, 14, 192))
    k = torch.rand((32, 14, 192))
     
    q = torch.rand((32, 14, 192))
